...
# Script to fix SDK dependency path in tv2demo
# This script updates the local package reference to point to the correct SDK location

set -e

PROJECT_FILE="tv2demo.xcodeproj/project.pbxproj"
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"

cd "$SCRIPT_DIR"

echo "🔧 Fixing SDK dependency path in tv2demo..."
...
if [ -f "tv2demo.xcodeproj/project.xcworkspace/xcshareddata/swiftpm/Package.resolved" ]; then
    rm "tv2demo.xcodeproj/project.xcworkspace/xcshareddata/swiftpm/Package.resolved"
    echo "✅ Removed old Package.resolved"
fi

echo ""
echo "✅ Done! Next steps:"
echo "1. Open tv2demo.xcodeproj in Xcode"
...
echo "  - ReachuComplete (includes all SDK modules)"
