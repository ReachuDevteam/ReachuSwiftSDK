...
class RProductStoreViewModel: ObservableObject {
    ...
    func loadProducts(mode: String, productIds: [Int]?, currency: String, country: String) async {
        ...
        VioLogger.debug("Loading products - Mode: \(mode)", component: "RProductStore")
        ...
        do {
            ...
            if shouldUseFiltered {
                VioLogger.debug("Filtered mode - Product IDs: \(productIds ?? [])", component: "RProductStore")
                idsToUse = productIds
            } else if mode == "filtered" && !hasValidIds {
                // Filtered mode but no IDs - fallback to all products
                VioLogger.warning("Filtered mode requires product IDs but none provided - falling back to all products", component: "RProductStore")
                idsToUse = nil
            } else {
                // All mode - load all products
                VioLogger.debug("All mode - Loading all products from channel", component: "RProductStore")
                idsToUse = nil
            }
            ...
            if products.isEmpty && mode == "filtered" && hasValidIds {
                VioLogger.warning("No products found for filtered IDs: \(productIds ?? []) - falling back to all products", component: "RProductStore")
            ...
            VioLogger.error("Invalid configuration: \(message)", component: "RProductStore")
        } catch ProductServiceError.sdkError(let error) {
            if error.code == "NOT_FOUND" || error.status == 404 {
                isMarketUnavailable = true
                errorMessage = nil
                VioLogger.warning("Market not available", component: "RProductStore")
            } else {
                errorMessage = error.message
                VioLogger.error("Failed to load products: \(error.message)", component: "RProductStore")
            }
        } catch ProductServiceError.networkError(let error) {
            errorMessage = error.localizedDescription
            VioLogger.error("Network error: \(error.localizedDescription)", component: "RProductStore")
        } catch {
            errorMessage = error.localizedDescription
            VioLogger.error("Failed to load products: \(error.localizedDescription)", component: "RProductStore")
        }
