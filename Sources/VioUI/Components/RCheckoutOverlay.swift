...
public struct RCheckoutOverlay: View {
        ...
        NavigationView {
            ...
            .onChange(of: checkoutStep) { newStep in
                if newStep == .orderSummary {
                    ...
                    Task { @MainActor in
                        VioLogger.debug("Payment successful - resetting cart and creating new one", component: "RCheckoutOverlay")
                        isLoading = true
    ...
    public var body: some View {
        ...
        if !VioConfiguration.shared.shouldUseSDK || !CampaignManager.shared.isCampaignActive {
            ...
            mainContent
            .onAppear {
                VioLogger.debug("onAppear triggered", component: "RCheckoutOverlay")
                syncSelectedMarket()
                Task { @MainActor in
                    isLoading = true
               ...
               !klarnaSelectedCategoryIdentifier.isEmpty {
                HiddenKlarnaAutoAuthorize(
                    ...
                    onAuthorized: { authToken, finalizeRequired in
                        Task { @MainActor in
                            VioLogger.debug("Step 5: Usuario autorizó el pago en Klarna", component: "RCheckoutOverlay")
                            VioLogger.debug("AuthToken (primeros 20): \(authToken.prefix(20))...", component: "RCheckoutOverlay")
                            VioLogger.debug("FinalizeRequired: \(finalizeRequired)", component: "RCheckoutOverlay")
                            VioLogger.debug("Step 6: Llamando a backend para confirmar pago", component: "RCheckoutOverlay")
                            ...
                            VioLogger.debug("CheckoutId: \(cartManager.checkoutId ?? "nil"), Email: \(email)", component: "RCheckoutOverlay")
                            ...
                            guard let result = await cartManager.confirmKlarnaNative(
                                ...
                                VioLogger.error("Backend no pudo confirmar el pago", component: "RCheckoutOverlay")
                                VioLogger.error("Verificar: AuthToken válido, Backend respondió, Klarna API respondió", component: "RCheckoutOverlay")
                                VioLogger.error("Setting checkoutStep to .error (Klarna confirm failed)", component: "RCheckoutOverlay")
                            ...
                            VioLogger.success("Step 7: PAGO EXITOSO - OrderId: \(result.orderId), FraudStatus: \(result.fraudStatus)", component: "RCheckoutOverlay")
                    ...
                    onFailed: { message in
                        Task { @MainActor in
                            VioLogger.error("Pago falló o fue cancelado - Mensaje: \(message)", component: "RCheckoutOverlay")
                            VioLogger.error("Razones posibles: Usuario canceló, Klarna rechazó, Error de red, Token expiró", component: "RCheckoutOverlay")
            ...
            .sheet(
                    ...
                    !klarnaSelectedCategoryIdentifier.isEmpty
                {
                        ...
                        onAuthorized: { authToken, finalizeRequired in
                            Task { @MainActor in
                                ...
                                if let confirmation = result {
                                    ...
                                    VioLogger.error("Setting checkoutStep to .error (proceedToNext failed)", component: "RCheckoutOverlay")
                                    checkoutStep = .error
                                }
                    ...
                    .padding()
                    .onAppear {
                        VioLogger.error("Setting checkoutStep to .error (KlarnaNativePaymentSheet onAppear)", component: "RCheckoutOverlay")
                        showKlarnaNativeSheet = false
                        checkoutStep = .error
                    }
                }
    ...
    private var orderSummaryStepView: some View {
        VStack(spacing: 0) {
            ...
            VStack(spacing: VioSpacing.sm) {
                ...
                Button(action: {
                    Task { @MainActor in
                        VioLogger.debug("Botón 'Initiate Payment' presionado - selectedPaymentMethod: \(selectedPaymentMethod.rawValue)", component: "RCheckoutOverlay")
                        
                        #if os(iOS)
                            VioLogger.debug("Platform: iOS detected", component: "RCheckoutOverlay")
                            if selectedPaymentMethod == .stripe {
                                ...
                                if ok {
                                    ...
                                    VioLogger.error("Setting checkoutStep to .error (Stripe prepareStripePaymentSheet failed)", component: "RCheckoutOverlay")
                                    checkoutStep = .error
                                    return
                                }
                            }
                            if selectedPaymentMethod == .klarna {
                                VioLogger.debug("Botón 'Initiate Payment' presionado con Klarna seleccionado - Llamando a initiateKlarnaDirectFlow()", component: "RCheckoutOverlay")
                                // Usar flujo directo de Klarna sin UI intermedia
                                await initiateKlarnaDirectFlow()
                                return
                            }
                            if selectedPaymentMethod == .vipps {
                                VioLogger.debug("Botón 'Initiate Payment' presionado con Vipps seleccionado - Llamando a initiateVippsFlow()", component: "RCheckoutOverlay")
                                // Usar flujo directo de Vipps
                                await initiateVippsFlow()
                                return
                            }
                        #else
                            VioLogger.warning("Platform: NO ES iOS - saltando lógica de pago", component: "RCheckoutOverlay")
                        #endif
                        VioLogger.debug("Llamando a proceedToNext()", component: "RCheckoutOverlay")
                        proceedToNext()
                    }
                }) {
                    HStack {
    ...
    private func proceedToNext() {
        VioLogger.debug("proceedToNext FUNCIÓN LLAMADA - checkoutStep actual: \(checkoutStep), selectedPaymentMethod: \(selectedPaymentMethod.rawValue)", component: "RCheckoutOverlay")
        withAnimation(.easeInOut(duration: 0.3)) {
            switch checkoutStep {
            case .address:
                checkoutStep = .orderSummary
            case .orderSummary:
                ...
                if selectedPaymentMethod == .klarna {
                    #if os(iOS) && canImport(KlarnaMobileSDK)
                        VioLogger.debug("Klarna detectado en orderSummary - Llamando a initiateKlarnaDirectFlow()", component: "RCheckoutOverlay")
                        Task {
    ...
    private func formattedOptionName(_ name: String) -> String {
        ...
        private func initiateKlarnaDirectFlow() async {
            VioLogger.debug("Klarna Flow INICIO - Step 1: Preparando datos del checkout", component: "RCheckoutOverlay")
            ...
            VioLogger.debug("Datos preparados: Email=\(email), País=\(country)→\(countryCode), Moneda=\(cartManager.currency), Locale=\(locale), CheckoutId=\(cartManager.checkoutId ?? "nil")", component: "RCheckoutOverlay")
            ...
            VioLogger.debug("Step 2: Llamando a backend Reachu (initKlarnaNative)", component: "RCheckoutOverlay")
            
            // Call backend to initialize Klarna session
            guard let dto = await cartManager.initKlarnaNative(input: input) else {
                VioLogger.error("initKlarnaNative returned: NIL - Backend retornó nil. Verificar: CheckoutId existe, Backend respondió, Credenciales configuradas", component: "RCheckoutOverlay")
                await MainActor.run {
                    VioLogger.error("Setting checkoutStep to .error (initKlarnaNative returned nil)", component: "RCheckoutOverlay")
            ...
            VioLogger.success("Step 3: Backend respondió correctamente - SessionId: \(dto.sessionId), ClientToken: \(dto.clientToken.prefix(20))..., Categorías: \(dto.paymentMethodCategories?.count ?? 0)", component: "RCheckoutOverlay")

            await MainActor.run {
                // Backend already returns the correct DTO structure
                let categories = dto.paymentMethodCategories ?? []
                guard !categories.isEmpty else {
                    VioLogger.error("ERROR: No hay métodos de pago disponibles - Setting checkoutStep to .error", component: "RCheckoutOverlay")
                    self.isLoading = false
                ...
                VioLogger.debug("Métodos de pago disponibles: \(categories.map { "\($0.identifier): \($0.name ?? "sin nombre")" }.joined(separator: ", "))", component: "RCheckoutOverlay")
                ...
                if let firstCategory = categories.first {
                    ...
                    VioLogger.debug("Categoría seleccionada: \(firstCategory.identifier)", component: "RCheckoutOverlay")
                }
                ...
                VioLogger.debug("Step 4: Activando auto-authorize (modal Klarna)", component: "RCheckoutOverlay")
    ...
    private func initiateVippsFlow() async {
        VioLogger.debug("Vipps Flow INICIO - Step 1: Preparando datos del checkout", component: "RCheckoutOverlay")
        ...
        VioLogger.debug("Datos preparados: Email=\(email), CheckoutId=\(checkoutId), Success URL=\(successUrlWithTracking), Cancel URL=\(cancelUrlWithTracking)", component: "RCheckoutOverlay")
        
        VioLogger.debug("Step 2: Llamando a backend Reachu (vippsInit)", component: "RCheckoutOverlay")
        ...
        guard let dto = await cartManager.vippsInit(
            ...
            returnUrl: successUrlWithTracking
                ...
                VioLogger.error("vippsInit returned: NIL - Backend retornó nil. Verificar: CheckoutId existe, Backend respondió, Credenciales configuradas", component: "RCheckoutOverlay")
                await MainActor.run {
                    VioLogger.error("Setting checkoutStep to .error (vippsInit returned nil)", component: "RCheckoutOverlay")
                self.isLoading = false
        ...
        VioLogger.success("Step 3: Backend respondió correctamente - Payment URL: \(dto.paymentUrl)", component: "RCheckoutOverlay")
        
        await MainActor.run {
            self.isLoading = false
            
            VioLogger.debug("Step 4: Abriendo Vipps en navegador", component: "RCheckoutOverlay")
            ...
            if let url = URL(string: dto.paymentUrl) {
                ...
                VioLogger.success("Vipps abierto en navegador - Payment marked as in progress", component: "RCheckoutOverlay")
            } else {
                VioLogger.error("ERROR: URL inválida", component: "RCheckoutOverlay")
    ...
    private func startVippsRetryTimer() {
        VioLogger.debug("Starting retry timer - Max retries: \(vippsMaxRetries)", component: "RCheckoutOverlay")
    ...
    private func stopVippsRetryTimer() {
        VioLogger.debug("Stopping retry timer", component: "RCheckoutOverlay")
        vippsRetryTimer?.invalidate()
        vippsRetryTimer = nil
    }
    ...
    private func checkVippsPaymentStatusWithRetry() async {
        ...
        vippsRetryCount += 1
        VioLogger.debug("Attempt \(vippsRetryCount)/\(vippsMaxRetries) - Checking status for checkout: \(checkoutId)", component: "RCheckoutOverlay")
        
        // Check checkout status from backend
        if let checkout = await cartManager.getCheckoutById(checkoutId: checkoutId) {
            VioLogger.debug("Checkout status: \(checkout.status ?? "unknown")", component: "RCheckoutOverlay")
            
            if checkout.status.uppercased() == "SUCCESS" {
                VioLogger.success("Payment successful!", component: "RCheckoutOverlay")
                await MainActor.run {
            ...
            if vippsRetryCount >= vippsMaxRetries {
                VioLogger.warning("Max retries reached. Payment not successful.", component: "RCheckoutOverlay")
                await MainActor.run {
                    self.stopVippsRetryTimer()
                    self.vippsPaymentInProgress = false
                    self.vippsCheckoutId = nil
            ...
            VioLogger.debug("Status not SUCCESS yet. Waiting for webhook... (\(vippsRetryCount)/\(vippsMaxRetries))", component: "RCheckoutOverlay")
            
        } else {
            VioLogger.error("Could not retrieve checkout status", component: "RCheckoutOverlay")
    ...
    private func handleVippsPaymentStatusChange(_ status: VippsPaymentHandler.PaymentStatus) {
        VioLogger.debug("Status changed to: \(status)", component: "RCheckoutOverlay")
        ...
        case .failed, .cancelled:
            VioLogger.error("Payment failed or cancelled", component: "RCheckoutOverlay")
            stopVippsRetryTimer()
        ...
        case .inProgress:
            VioLogger.debug("Payment in progress", component: "RCheckoutOverlay")
            // Keep current state and retry timer running
            
        case .unknown:
            VioLogger.warning("Unknown status", component: "RCheckoutOverlay")
        ...
        private func presentStripePaymentSheet() {
            ...
            sheet.present(from: root) { result in
                ...
                case .failed(let error):
                    VioLogger.error("Setting checkoutStep to .error (Stripe payment failed: \(error.localizedDescription))", component: "RCheckoutOverlay")
...
extension RCheckoutOverlay {
    ...
    private func loadCheckoutTotals() async {
        guard let checkoutId = cartManager.checkoutId else {
            VioLogger.debug("No checkoutId available to load totals", component: "RCheckoutOverlay")
            return
        }
        
        VioLogger.debug("Loading checkout totals for checkoutId: \(checkoutId)", component: "RCheckoutOverlay")
        
        if let checkout = await cartManager.getCheckoutById(checkoutId: checkoutId) {
            await MainActor.run {
                checkoutTotals = checkout
                VioLogger.debug("Checkout totals loaded - shipping: \(checkout.totals?.shipping ?? 0), taxes: \(checkout.totals?.taxes ?? 0)", component: "RCheckoutOverlay")
            }
        } else {
            VioLogger.debug("Failed to load checkout totals", component: "RCheckoutOverlay")
        }
    }
    
    private func loadAvailablePaymentMethods() async {
        VioLogger.debug("Loading available payment methods...", component: "RCheckoutOverlay")
        
        // 1. Get supported methods from config
        let configMethods = VioConfiguration.shared.cartConfiguration.supportedPaymentMethods
        VioLogger.debug("Config supported methods: \(configMethods)", component: "RCheckoutOverlay")
        ...
        guard let baseURL = URL(string: config.environment.graphQLURL) else {
            VioLogger.error("Invalid GraphQL URL", component: "RCheckoutOverlay")
        ...
        do {
            ...
            VioLogger.info("API returned \(apiMethods.count) payment methods", component: "RCheckoutOverlay")
            ...
            for apiMethod in apiMethods {
                ...
                VioLogger.debug("API method: \(apiMethod.name) (normalized: \(methodName))", component: "RCheckoutOverlay")
                ...
                if let paymentMethod = PaymentMethod(rawValue: methodName) {
                    ...
                    VioLogger.debug("Added: \(methodName)", component: "RCheckoutOverlay")
                } else {
                    VioLogger.warning("Unknown payment method (no enum case): \(methodName)", component: "RCheckoutOverlay")
                }
            }
            ...
            await MainActor.run {
                ...
                if let first = available.first {
                    ...
                    VioLogger.debug("Auto-selected: \(first.rawValue)", component: "RCheckoutOverlay")
                }
                
                VioLogger.debug("Final available methods: \(available.map { $0.rawValue })", component: "RCheckoutOverlay")
            }
            
        } catch {
            VioLogger.error("Failed to fetch payment methods: \(error)", component: "RCheckoutOverlay")
            await setFallbackPaymentMethods(configMethods)
        }
    }
    ...
    private func setFallbackPaymentMethods(_ configMethods: [String]) async {
        await MainActor.run {
            ...
            VioLogger.debug("Using config fallback: \(fallbackMethods.map { $0.rawValue })", component: "RCheckoutOverlay")
        }
    }
...
// MARK: - Preview
    ...
    struct HiddenKlarnaAutoAuthorize: UIViewRepresentable {
        ...
        class Coordinator: NSObject, KlarnaPaymentEventListener {
            ...
            func klarnaInitialized(paymentView: KlarnaPaymentView) {
                VioLogger.debug("Initialized", component: "RCheckoutOverlay")
            }
            
            func klarnaLoaded(paymentView: KlarnaPaymentView) {
                VioLogger.debug("Loaded", component: "RCheckoutOverlay")
            }
            
            func klarnaLoadedPaymentReview(paymentView: KlarnaPaymentView) {
                VioLogger.debug("Loaded payment review", component: "RCheckoutOverlay")
            }
            ...
            func klarnaAuthorized(
                ...
                VioLogger.debug("Authorized - approved: \(approved), token: \(authToken != nil)", component: "RCheckoutOverlay")
                guard approved, let token = authToken, !token.isEmpty else {
                    DispatchQueue.main.async {
            ...
            func klarnaReauthorized(
                ...
                VioLogger.debug("Reauthorized", component: "RCheckoutOverlay")
            }
            ...
            func klarnaFinalized(
                ...
                VioLogger.debug("Finalized", component: "RCheckoutOverlay")
            }
            ...
            func klarnaFailed(
                ...
                VioLogger.error("Failed: \(error.localizedDescription)", component: "RCheckoutOverlay")
                DispatchQueue.main.async {
                    self.parent.onFailed(error.localizedDescription)
                }
            }
