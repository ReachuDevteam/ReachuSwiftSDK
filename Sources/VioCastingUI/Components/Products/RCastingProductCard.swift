...
struct CastingProductCard: View {
    ...
    private func configProductId(for product: Product) -> String? {
        ...
        if DemoDataManager.shared.productUrl(for: productIdString) != nil {
    ...
    private func getProductUrl(for product: Product) -> String? {
        guard let configId = configProductId(for: product) else {
            let fallback = productEvent.castingProductUrl ?? ""
            return fallback.isEmpty ? nil : fallback
        }
        return DemoDataManager.shared.productUrl(for: configId)
            ?? (productEvent.castingProductUrl?.isEmpty == false ? productEvent.castingProductUrl : nil)
    }
    
    private func getCheckoutUrl(for product: Product) -> String? {
        guard let configId = configProductId(for: product) else {
            let fallback = productEvent.castingCheckoutUrl ?? ""
            return fallback.isEmpty ? nil : fallback
        }
        return DemoDataManager.shared.checkoutUrl(for: configId)
            ?? (productEvent.castingCheckoutUrl?.isEmpty == false ? productEvent.castingCheckoutUrl : nil)
    }
